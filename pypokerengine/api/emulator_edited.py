...
class Emulator(object):
    ...
    def register_player(self, uuid, player):
        ...
        player.declare_action = timeout2(0.0,default_action_info)(player.declare_action)
    ...
    def get_hole_cards(self, game_state):
        if game_state["next_player"] is None:
            return None
        ...
        hole_cards_str = [str(card) for card in hole_cards]
        
        return hole_cards_str
    ...
    def start_new_round(self, game_state):
        ...
        if len([1 for p in deepcopy_table.seats.players if p.is_active()])==1 or round_count > self.game_rule["max_round"]:
