...
from typing import List, Dict, Optional, Tuple, Set, get_args, cast
import json
import csv
import os

from notebook import TypedPokerPlayer, ActionOption, CardString, RoundState, Action, Seat, HandInfo
...
def simulate_game(path: Path) -> Tuple[Set[Action], List[Path]]:
    logger.debug(f"Simulation - Path: {path}")
    move_idx = 0
    ...
    def cb(valid_actions: List[ActionOption], hole_card: List[CardString], round_state: RoundState) -> Action:
        ...
        if move_idx < len(path):
            action = path[move_idx]
            assert action in get_args(Action), f"Invalid action type: {action}"
            action = cast(Action, action)
            assert action in valid, f"Action {action} not in valid actions: {actions}"
            logger.debug(f"Replaying action: {action} at path: {path}")
        else:
            if move_idx == len(path):
                for a in valid:
                    actions.add(a)
                    children.append(path + (a,))
                    logger.debug(f"Adding child path: {path + (a,)}")

            logger.debug(f"Exit by folding at path: {path}")
            action: Action = "fold"
    ...
    logger.debug(f"Simulation ended with path: {path}, actions: {actions}, children: {children}")
...
def main():
    ...
    #
    # # Export DOT
    # with open("game_tree.dot", "w") as f:
    ...
    # with open("game_tree.csv", "w", newline="") as f:
    #     writer = csv.writer(f)
